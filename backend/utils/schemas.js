...
export const createCanonFactSchema = Joi.object({
    ...
    source_episode: Joi.string().max(255).optional().allow("", null),
    embedding: Joi.array().items(Joi.number()).optional().allow(null),
    superseded_by: uuid().optional().allow(null),
    author_name: Joi.string().max(255).optional().allow("", null)
});
...
const SUBMISSION_STATUSES = ["pending", "analysed", "approved", "rejected"];
...
const CONFLICT_STATUSES = ["unresolved", "resolved", "dismissed"];
